...
export function NotificationBell() {
  ...
  // Add/remove body class when orders are paused (so navbar can shift down via CSS)
  useEffect(() => {
    if (!storeStatus.accepting) {
      document.documentElement.classList.add('orders-paused')
    } else {
      document.documentElement.classList.remove('orders-paused')
    }
    return () => document.documentElement.classList.remove('orders-paused')
  }, [storeStatus.accepting])

  return (
    <>
      {/* Store Status Banner (shows when orders paused) — at very top, pushes navbar down */}
      {!storeStatus.accepting && (
        <div className="fixed left-0 right-0 top-0 z-[55] border-b border-[#FF2D55]/40 bg-[#FF2D55]/10 backdrop-blur-md animate-slide-down">
